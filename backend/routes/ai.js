...
// ─── GET /api/ai/geojson/:code ────────────────────────────────────────────────
// Serve cached GeoJSON from db.json (no AI call needed)

router.get('/geojson/:code', (req, res) => {
  try {
    const db      = JSON.parse(fs.readFileSync(path.join(__dirname, '../db.json'), 'utf-8'));
    const geojson = db.geojson?.[req.params.code.toUpperCase()];
    if (!geojson) {
      return res.status(404).json({ error: `GeoJSON for ${req.params.code} not found. Run generate-routes.js first.` });
    }
...
// ─── GET /api/ai/geojson ──────────────────────────────────────────────────────
// Serve ALL cached GeoJSON at once

router.get('/geojson', (req, res) => {
  try {
    const db = JSON.parse(fs.readFileSync(path.join(__dirname, '../db.json'), 'utf-8'));
    res.json({ success: true, geojson: db.geojson || {} });
  } catch (err) {
    res.status(500).json({ error: 'Failed to load GeoJSON' });
  }
});
